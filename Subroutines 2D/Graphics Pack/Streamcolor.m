...
%--------------------------------------------------------------------------
% Edited by Vadim Malis
% 03/2015 UCSD RIL



lw=.03; % width of the streamlines
cmap=colormap;
...
caxis([0,Vmax])
set(h,'ylim',[0,Vmax]);
colorbar_scale=roundn((Vmax)/5,2);
colorbar_scale_r=roundn(colorbar_scale,1);
set(h,'YTick',[0,0+colorbar_scale:colorbar_scale_r:Vmax]);
xlabel(h,'s^{-1}','FontSize', 14);
set(gca,'FontSize', 12);
cb_pos=get(h,'Position');
set(h,'Position',[cb_pos(1)*0.64,cb_pos(2)*.5,cb_pos(3)*1.16,cb_pos(4)*.3])
