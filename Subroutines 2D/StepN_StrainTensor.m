%=========================================================================
%   StepN for 2d Strain Rate Analysis Toolbox (master script)
%
%       this script automaticaly does all aditional steps needed for
%       calucluation of strain rate tensor components in the basis of
%       muscle fibers
%
...
%for i=1
    ...
    % forth dimension is the region (proximal, middle, distal)
    for j=1:size(MASK,4)
         ...
         SR_ratio_temp=zeros(size(i_3,1),1);
         ...
         for k=1:size(i_1,1)       
             ...
             SR_ratio_temp(k,1)=abs(SR_temp(2,2,k))/abs(SR_temp(1,2,k));
         end   
         ...
         % k is the loop through the frames
         for k=1:size(a,1)
             SR_tensor(1,1,k,j)=stdfilter(squeeze(SR_temp(1,1,b(k):b(k+1))));   
             SR_tensor(2,2,k,j)=stdfilter(squeeze(SR_temp(2,2,b(k):b(k+1))));   
             SR_tensor(1,2,k,j)=stdfilter(squeeze(SR_temp(1,2,b(k):b(k+1))));  
             SR_tensor(2,1,k,j)=SR_tensor(1,2,k,j);             
             SR_ratio(k,j)=stdfilter(squeeze(SR_ratio_temp(b(k):b(k+1),1)));
         end
...
save('SR_tensor_abs.mat','SR_data');    
...
for i=1:size(DATA,2)
    ...
    SR_peak_temp(j).SR_ff=SR_data_sorted(i).SR_tensor(2,2,DATA(i).peak_ev_frame(j),j);
    SR_peak_temp(j).SR_cc=SR_data_sorted(i).SR_tensor(1,1,DATA(i).peak_ev_frame(j),j);
    SR_peak_temp(j).SR_fc=SR_data_sorted(i).SR_tensor(1,2,DATA(i).peak_ev_frame(j),j);
    SR_peak_temp(j).SR_ratio=SR_data_sorted(i).SR_ratio(DATA(i).peak_ev_frame(j),j);
    end
...
for i=1:size(DATA,2)
    ...
    SR_sfl_temp(j).SR_ff=SR_data_sorted(i).SR_tensor(2,2,DATA(i).force_level_frame,j);
    SR_sfl_temp(j).SR_cc=SR_data_sorted(i).SR_tensor(1,1,DATA(i).force_level_frame,j);
    SR_sfl_temp(j).SR_fc=SR_data_sorted(i).SR_tensor(1,2,DATA(i).force_level_frame,j);
    SR_sfl_temp(j).SR_ratio=SR_data_sorted(i).SR_ratio(DATA(i).force_level_frame,j);
...
for i=1:size(DATA,2)
    ...
    SR_sfl_PP_temp(j).SR_ff=SR_data_sorted(i).SR_tensor(2,2,DATA(i).post_peak_same_force_frame(j),j);
    SR_sfl_PP_temp(j).SR_cc=SR_data_sorted(i).SR_tensor(1,1,DATA(i).post_peak_same_force_frame(j),j);
    SR_sfl_PP_temp(j).SR_fc=SR_data_sorted(i).SR_tensor(1,2,DATA(i).post_peak_same_force_frame(j),j);
    SR_sfl_PP_temp(j).SR_ratio=SR_data_sorted(i).SR_ratio(DATA(i).post_peak_same_force_frame(j),j);
    end
...
for i=1:size(Unique_patientID_index,1)-1
    ...
    SR_peak_averaged(i).SR_ratio=stdfilter([SR_peak(Unique_patientID_index(i):Unique_patientID_index(i+1)-1).SR_ratio]');
...
%--------------------------------------------------------------------------
    ...
    SR_sfl_averaged(i).SR_ratio=stdfilter([SR_sfl(Unique_patientID_index(i):Unique_patientID_index(i+1)-1).SR_ratio]');
...
%--------------------------------------------------------------------------
    ...
    SR_sfl_PP_averaged(i).SR_ratio=stdfilter([SR_sfl_PP(Unique_patientID_index(i):Unique_patientID_index(i+1)-1).SR_ratio]');
...
save('Extracted_SR_data_correct_STDFILT.mat','SR_peak')
save('Extracted_SR_data_correct_STDFILT.mat','SR_peak_averaged','-append')
save('Extracted_SR_data_correct_STDFILT.mat','SR_sfl','-append')
save('Extracted_SR_data_correct_STDFILT.mat','SR_sfl_averaged','-append')
save('Extracted_SR_data_correct_STDFILT.mat','SR_sfl_PP','-append')
save('Extracted_SR_data_correct_STDFILT.mat','SR_sfl_PP_averaged','-append')
%finaly preparing arrays for plots


%% average results per group and obtain values at max frame and same force level
