function [diff_im] = anisodiff2D(im,im_mag, num_iter, delta_t, kappa, option)
...
%USHA threshold by magnitude image, im
%indices=find(im_mag<50);
%im(indices)=0;
%USHA

% Convert input phase image to double, im.
im = double(im);

% USHA Convert input phase image to double, im_phase
%im_phase = double(im_phase);
%USHA

% PDE (partial differential equation) initial condition.
diff_im = im;
%diff_imphase = im_phase; 
